...
sub BigHash
{
    ...
    {
       chomp ($repeat);
       $repeat =~ /(\S+)\s+(\d+)\s+(\d+).*?(C|\+)\s+\S+\s+(\S+)/;  ## I have question here ? $1
...
sub ExtractRepeats
{
    ...
    if (!defined($category))
    {
        $category = "./FamilyCategorization.txt";
    }
    ...
    while(defined(my $line = <SUPERFASTA>)) 
    {
        if ($line =~ /^>(\S+)/) {
            my $newname = $1;
            if ($contigname) {
                &process($contigname,$sequence,\%filehandles);   ## pass hashtable by reference
            }
...
sub process
{
    ...
    while (defined($item = shift @bighashvalues))
    {
...
sub Help
{
    ...
    Masterfile          = Masterfile of some species
